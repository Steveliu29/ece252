...
CURL *easy_handle_init(RECV_BUF *ptr, const char *url, int index)
{
    ...
    curl_easy_setopt(curl_handle, CURLOPT_PRIVATE, index);
...
int main( int argc, char** argv )
{
    ...
    cm = curl_multi_init();
    RECV_BUF* recv_buf = malloc (t * sizeof(RECV_BUF));
    memset(recv_buf, 0, t * sizeof(RECV_BUF));
    ...
    while (!is_empty(url_queue) && URL_counter != m){
        ...
        while ((msg = curl_multi_info_read(cm, &msgs_left))) {
            ...
            if (msg->msg == CURLMSG_DONE) {
                ...
                //     printf("200 OK for %s\n", szUrl);
                if (URL_counter != m)
                    process_data(eh, &recv_buf[curl_index], url_queue, my_png_url);
                // } else {
